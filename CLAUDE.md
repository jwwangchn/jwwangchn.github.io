...
- `_site/` and `.jekyll-cache/` are generated by local Jekyll builds. Do not treat them as source content.

## Source Layout

| Area | Path | Purpose |
|------|------|---------|
| Content | `_pages/about.md` | Homepage biography, featured projects, honors, and publication toggle |
| Content | `_bibliography/papers.bib` | Publication data |
| Content | `_data/socials.yml` | Email, GitHub, and Google Scholar identifiers |
| Page layouts | `_layouts/` | Jekyll layout shells for pages and publication cards |
| Site includes | `_includes/site/` | Head, header, footer, metadata, and script loading |
| Publication includes | `_includes/publications/` | Publication-specific helper includes |
| Shared includes | `_includes/shared/` | Reusable Liquid snippets |
